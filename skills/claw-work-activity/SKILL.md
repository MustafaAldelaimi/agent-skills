...
- **Capabilities at this version (verified against `0.1.0a20`):** parsed records come from `agentgrep search <terms> --scope conversations --json` (term-driven, rapidfuzz-ranked) — `find` only lists store *paths*, not conversations, so there is **no "list everything in a window" mode**. Fields are uneven: `title` is null for every agent, `timestamp` is populated only for `cursor-cli`, `session_id` only for `claude`. Treat agentgrep as **cross-agent keyword recall**, not a titled/windowed session enumerator.
- **Cursor IDE stays on the SQLite path above** (authoritative titles + timestamps). agentgrep's cursor-ide adapter currently surfaces only the flat `aiService.prompts` list (null title/timestamp), so do not use it for Cursor IDE.
- **Other agents (Codex / Claude / Cursor CLI / Gemini / ...):** run `agentgrep search <terms> --scope conversations --agent all --json`. Where `timestamp` is present (e.g. `cursor-cli`) filter to the window; otherwise present hits as undated recall. Each hit carries `agent`, `path` (the file for the subagent to read) and `text`; when `title` is null, derive a label from the first line of `text`.
- **Dedupe against the SQLite rows:** a Cursor IDE hit from agentgrep and the same `composerData` row are one session — the SQLite row wins (exact title). Non-Cursor agents add net-new sessions.
